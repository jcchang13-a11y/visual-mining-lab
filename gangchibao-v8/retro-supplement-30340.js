/* 《剛吃飽》第八版｜爛尾樓版｜30340 回溯增補
 * 忠於正文既有回指，不改正文。
 */
(function(){
  'use strict';
  const unit=new URLSearchParams(location.search).get('u');
  if(unit!=='30340') return;
  const article=document.getElementById('article');
  if(!article) return;

  const key='retro-30340-13-5';
  const anchor='這一段看起來很像重複第五分。';
  const caption='第十三分以「三十二相能不能見如來」明確回收第五分「可以身相見如來不」；焦點已換，但回溯成立：13 → 5';
  ...
  function makeFigure(){
    ...
    img.src='figures/retro-30340-13-5.svg';
    img.alt=caption;
    img.loading='lazy';
    img.decoding='async';
