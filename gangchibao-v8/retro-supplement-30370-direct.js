/* 《剛吃飽》第八版｜爛尾樓版｜30370 第十五分直接回溯增補
 * 不清場，只加鷹架：保留既有 15→14→13、15→13→11→8 與 15→12；
 * 只把正文自己明確叫回的前段位置拆成直接同心圓。
 */
(function(){
  'use strict';
  ...
  const specs=[
    ...
    },
    {
      ...
      caption:'第十五分的身布施比較直接承接第十四分「第一波羅蜜即非第一波羅蜜／忍辱波羅蜜非忍辱波羅蜜」的現場：15 → 14',
      anchor:'第十五分到這裡，不再只是說兩邊都有福，而是說得很清楚：其福勝彼。'
    },
    {
  ...
  function apply(){
    let done=true;
    for(const spec of specs) if(!insertAfterAnchor(spec)) done=false;
    return done;
  }
