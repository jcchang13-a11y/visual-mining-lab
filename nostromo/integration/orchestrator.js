/* NOSTROMO integration orchestrator v0.2 */
(function(root){
  ...
  function makeRequests(live,round,seed){
    if(!root.NostromoActions)return [];
    return [
      root.NostromoActions.request('shrooming','POSE_QUESTION',{question:`Round ${round}: 哪個既有判斷最值得重新讀？`,seed},{round,source:'NOSTROMO'}),
      root.NostromoActions.request('muther','FALSIFY',{candidate:live.muther?.terminus?.candidate||null,seed},{round,source:'NOSTROMO'}),
      root.NostromoActions.request('droplet','VERIFY',{claim:live.droplet?.summary||null,seed},{round,source:'NOSTROMO'})
    ];
  }
  async function runRound(seed,round){
    ...
    const actionRequests=makeRequests(live,round,seed);
    const gutOut=root.GutEngine.digest({round,seed,gutIn,vajra,actionRequests},{source:'NOSTROMO/round-output'});
    const requestSummary=actionRequests.reduce((a,x)=>{a[x.status]=(a[x.status]||0)+1;return a;},{});
    return {round,status:'PASS',sources:{shroomingUpdated:live.shrooming.updatedAt||null,mutherUpdated:live.muther.updatedAt||null,dropletUpdated:live.droplet.updatedAt||null},routing:['SHROOMING','MU/TH/UR','DROPLET','GUT','VAJRA','ACTION ADAPTERS','GUT'],gut:{in:{ingested:gutIn.ingested,absorbed:gutIn.absorbed,excreted:gutIn.excreted},out:{ingested:gutOut.ingested,absorbed:gutOut.absorbed,excreted:gutOut.excreted}},vajra:{status:vajra.status,traceLength:vajra.trace.length},actions:{count:actionRequests.length,summary:requestSummary,requests:actionRequests.map(x=>({organ:x.envelope?.organ||null,action:x.envelope?.action||null,status:x.status,reason:x.reason||null}))},carry:gutOut.summary.slice(0,1200)};
  }
