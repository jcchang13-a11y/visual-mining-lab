...
function run(label,text,expectedType,requiredLenses=[]){const out=globalThis.VajraEngine.run(text,6);check(out.version==='0.2.1',`${label}_VERSION`,out.version);check(out.positionType===expectedType,`${label}_TYPE`,{expectedType,actual:out.positionType});check(out.trace.length>=1,`${label}_EMPTY_TRACE`,out);for(const lens of requiredLenses)check(out.lensesUsed.includes(lens),`${label}_MISSING_LENS`,{lens,lensesUsed:out.lensesUsed});check(out.unresolved.length===out.trace.length,`${label}_UNRESOLVED_TRACE_MISMATCH`,{unresolved:out.unresolved.length,trace:out.trace.length});check(!['PROVED','TRUE','FALSE','SETTLED','VERIFIED'].includes(out.status),`${label}_FALSE_CERTAINTY_STATUS`,out.status);check(out.unresolved.every(x=>x.status==='UNRESOLVED'),`${label}_FALSE_CERTAINTY_BRANCH`,out.unresolved);check(/^[0-9a-f]{8}$/.test(out.targetRef||''),`${label}_TARGET_REF_MISSING`,out.targetRef);return out;}
