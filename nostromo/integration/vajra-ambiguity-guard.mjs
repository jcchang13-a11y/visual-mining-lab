// VAJRA receipt ambiguity/conflict guard v0.5.4
...
// concessive scope phrases. Conditionality/exception coverage includes bounded prefix
// forms without crossing sentence or semicolon boundaries. Concessive coverage masks
// directional vocabulary only inside a bounded comma-delimited concessive prefix, so
// a later main-clause directional statement remains visible. This is deterministic
...
function maskConcessivePolarityPhrases(text){
  ...
  let out=text.replace(/\b(?:although|though|even\s+though|despite|in\s+spite\s+of)\b[^.!?;；,，]{0,120}[,，]/g,mask);
  out=out.replace(/(?:雖然|儘管|即使)[^。！？；;,，]{0,120}[，,]/g,mask);
  return out;
}
function relationPolarity(text){
  const s=clean(text).normalize('NFKC').toLowerCase();
  const scan=maskConcessivePolarityPhrases(maskExceptionPolarityPhrases(maskConditionalPolarityPhrases(maskHedgedPolarityPhrases(maskNegatedPolarityPhrases(s)))));
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {...base,version:`${base.version||'unknown'}+ambiguity-conflict-guard-v0.5.4`,status,unresolved,handoffs:unresolved.map(x=>x.handoff),receiptAmbiguityAudit:audit,handoffResolution:{...(base.handoffResolution||{}),resolved:resolvedBranches.length,open:openBranches.length,contested:contestedBranches.length,ambiguous:audit.ambiguous.length,contestedBranches:contestedBranches.map(x=>x.contest),ambiguousBranches:audit.ambiguous},boundary:`${base.boundary||''} Ambiguity/conflict guard: branch-scoped qualifying executed returns are audited across organ labels. Explicit opposing polarity creates CONTESTED_BY_RECEIPTS and blocks preferred-organ first-receipt closure; an UNSPECIFIED relation creates AMBIGUOUS_BY_RECEIPTS. Obvious negated, explicitly insufficient, explicitly hedged, explicitly conditional, bounded exception-scoped, or bounded concessive-prefix support/refute phrases are masked before lexical polarity classification. Condition/exception prefix scans do not cross sentence or semicolon boundaries; concessive-prefix scans stop at the first comma so later main-clause polarity remains visible. Distinct organ/provenance labels remain audit metadata rather than proof of independence. This guard is lexical and structural, not semantic adjudication.`.trim()};
}
