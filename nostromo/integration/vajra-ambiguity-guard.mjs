...
function receiptAuditView(x){
  return {
    targetRef:x.targetRef,clauseRef:x.clauseRef,lens:x.lens,organ:x.organ,
    status:x.status,provenance:x.provenance,material:x.material,relation:x.relation,
    polarity:x.polarity,evidenceKey:fp(`${x.provenance}|${x.material}`)
  };
}
function branchAuditFinding(key,items,status,polarities,boundary){
  const organs=[...new Set(items.map(x=>x.organ).filter(Boolean))];
  const provenances=[...new Set(items.map(x=>x.provenance).filter(Boolean))];
  return {
    key,status,polarities,count:items.length,organs,
    distinctOrganCount:organs.length,distinctProvenanceCount:provenances.length,
...
export function auditReceiptAmbiguity(receipts=[]){
  ...
  for(const [key,items] of groups){
    ...
    if(explicitConflict){
      contested.push(branchAuditFinding(
        key,items,'CONTESTED_BY_RECEIPTS',polarities,
        'Multiple structurally qualifying executed returns address the same VAJRA branch and contain explicit opposing lexical polarity, including across organ labels. The branch must not close by preferred-organ arrival order. Distinct organ/provenance counts are audit metadata only; they do not prove source independence. This is a bounded lexical conflict detector, not semantic adjudication.'
      ));
      continue;
    }
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  const contestedKeys=new Set(audit.contested.map(x=>x.key));
  const unresolved=(base.unresolved||[]).map(branch=>{
    ...
    if(contestedKeys.has(key)){
      const finding=audit.contested.find(x=>x.key===key);
      const contest={
        targetRef:clean(h.targetRef||branch.targetRef),
        clauseRef:clean(h.clauseRef||branch.clauseRef),
        lens:clean(h.lens||branch.lens),
        preferredOrgan:clean(h.preferredOrgan),
        status:'CONTESTED_BY_RECEIPTS',
        polarities:finding?.polarities||[],
        evidence:(finding?.receipts||[]).map(x=>({
          evidenceKey:x.evidenceKey,provenance:x.provenance,relation:x.relation,polarity:x.polarity,
          organ:x.organ,material:x.material
        })),
        boundary:'Branch-scoped explicit opposing receipt polarity is preserved across organ labels. A non-preferred organ return cannot close the branch, but it can block premature closure and create auditable conflict context. Distinct organs/provenances are not proof of source independence.'
      };
      return {...branch,status:'CONTESTED_BY_RECEIPTS',contest,handoff:{...h,status:'CONTESTED_BY_RECEIPTS'}};
    }
  ...
  const contestedBranches=unresolved.filter(x=>x.status==='CONTESTED_BY_RECEIPTS');
  ...
  return {
    ...
    handoffResolution:{
      ...(base.handoffResolution||{}),
      resolved:resolvedBranches.length,
      open:openBranches.length,
      contested:contestedBranches.length,
