// VAJRA receipt ambiguity guard v0.3
...
// non-executed returns are excluded. This is deterministic audit logic, not
// semantic adjudication, source-independence proof or source-quality scoring.
  ...
  for(const [key,items] of groups){
    ...
    if(polarities.includes('UNSPECIFIED')){
      ...
      ambiguous.push({
        ...
        boundary:'Multiple structurally qualifying executed returns address the same VAJRA branch, but at least one relation is lexically UNSPECIFIED. Branch scope deliberately crosses organ labels so arrival order across organs cannot manufacture closure. Distinct organ/provenance counts are audit metadata only; they do not prove source independence. Malformed, non-executed, provenance-free, empty-material or relation-free returns cannot manufacture ambiguity. This is a lexical safety rule, not semantic disagreement detection.'
      });
    }
  }
  return {
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {
    ...base,
    version:`${base.version||'unknown'}+ambiguity-guard-v0.3`,
