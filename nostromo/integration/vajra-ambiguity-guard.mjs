// VAJRA receipt ambiguity/conflict guard v0.5.7
...
// prefixes (if/when/as long as and 如果/若/倘若/只要) now also contain directional vocabulary in
// the same conditional sentence, including the main clause, so a condition cannot be flattened into
// unconditional polarity. A later sentence remains visible. Concessive/contrastive coverage keeps
// its bounded prefix behavior. This is deterministic audit logic, not semantic adjudication,
...
function maskSimpleConditionalPrefixPolarityPhrases(text){
  ...
  let out=text.replace(/\b(?:if|when|as\s+long\s+as)\b[^.!?;；,，]{0,120}[,，]/g,mask);
...
function maskConditionalSentencePolarityPhrases(text){
  ...
  let out=text.replace(/\b(?:if|when|as\s+long\s+as)\b[^.!?;；]{0,220}(?=[.!?;；]|$)/g,(segment)=>/[,，]/.test(segment)?mask(segment):segment);
...
function maskConcessivePolarityPhrases(text){
  ...
  let out=text.replace(/\b(?:although|though|even\s+though|despite|in\s+spite\s+of|while|whilst|whereas)\b[^.!?;；,，]{0,120}[,，]/g,mask);
  out=out.replace(/(?:雖然|儘管|即使|雖說|固然)[^。！？；;,，]{0,120}[，,]/g,mask);
  return out;
}
function relationPolarity(text){
  const s=clean(text).normalize('NFKC').toLowerCase();
  const scan=maskConcessivePolarityPhrases(maskExceptionPolarityPhrases(maskConditionalSentencePolarityPhrases(maskSimpleConditionalPrefixPolarityPhrases(maskConditionalPolarityPhrases(maskHedgedPolarityPhrases(maskNegatedPolarityPhrases(s)))))));
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {...base,version:`${base.version||'unknown'}+ambiguity-conflict-guard-v0.5.7`,status,unresolved,handoffs:unresolved.map(x=>x.handoff),receiptAmbiguityAudit:audit,handoffResolution:{...(base.handoffResolution||{}),resolved:resolvedBranches.length,open:openBranches.length,contested:contestedBranches.length,ambiguous:audit.ambiguous.length,contestedBranches:contestedBranches.map(x=>x.contest),ambiguousBranches:audit.ambiguous},boundary:`${base.boundary||''} Ambiguity/conflict guard: branch-scoped qualifying executed returns are audited across organ labels. Explicit opposing polarity creates CONTESTED_BY_RECEIPTS and blocks preferred-organ first-receipt closure; an UNSPECIFIED relation creates AMBIGUOUS_BY_RECEIPTS. Obvious negated, explicitly insufficient, explicitly hedged, explicitly conditional, bounded generic conditional-sentence, bounded exception-scoped, or bounded concessive/contrastive-prefix support/refute phrases are masked before lexical polarity classification. Generic if/when/as-long-as and 如果/若/倘若/只要 conditional sentences mask same-sentence directional polarity through the sentence/semicolon boundary, so condition-scoped main clauses cannot be misread as unconditional closure; later sentences remain visible. Concessive/contrastive-prefix scans still stop at the first comma so later main-clause polarity remains visible. Distinct organ/provenance labels remain audit metadata rather than proof of independence. This guard is lexical and structural, not semantic adjudication.`.trim()};
}
