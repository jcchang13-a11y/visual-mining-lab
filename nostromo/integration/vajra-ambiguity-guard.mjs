// VAJRA receipt ambiguity/conflict guard v0.5.16
...
// phrases, including explicit insufficiency, modality, conditionality, exception,
// concessive/contrastive scope, attributed/reported polarity, bounded quoted text,
// and bounded parenthetical asides with deterministic nesting support.
// Conditionality/exception coverage includes bounded prefix forms without crossing
// sentence or semicolon boundaries. Generic comma-delimited condition prefixes
// (if/when/as long as and 如果/若/倘若/只要) contain directional vocabulary in the same
// conditional sentence, including the main clause. English even if is excluded from
// the generic if matcher so its comma-delimited concessive prefix can be contained
// without hiding an explicit directional main clause. Bounded explicit if...then and
// 如果/若/倘若/只要...則/那么/那麼 forms are also contained even without a comma. A later
// sentence remains visible. Attributed source claims, including bounded 根據/依據 comma
// or colon-delimited forms, bounded quoted excerpts, and bounded parenthetical asides
// are kept auditable but cannot by themselves become VAJRA's unconditional branch
// polarity; a later direct assessment outside those scopes remains visible.
// Relation scanning preserves newline boundaries before parenthetical masking so a
// multiline parser-limit case cannot be silently converted into an in-bound single-line
// parenthetical by whitespace normalization.
// This is deterministic audit logic, not semantic adjudication, quotation/parenthesis
// parsing, source-independence proof or source-quality scoring.

function clean(v){return String(v??'').replace(/\s+/g,' ').trim();}
function cleanForPolarity(v){return String(v??'').normalize('NFKC').replace(/\r\n?/g,'\n').replace(/[^\S\n]+/g,' ').replace(/ *\n */g,'\n').trim();}
...
function maskAttributedPolarityPhrases(text){
  const support=/(?:\bsupport(?:s|ed|ing)?\b|\bconfirm(?:s|ed|ing)?\b|\bcorroborat(?:e|es|ed|ing)\b|\bconsistent\s+with\b|支持|佐證|印證|吻合|一致)/g;
  const refute=/(?:\brefut(?:e|es|ed|ing)?\b|\bcontradict(?:s|ed|ing)?\b|\boppose(?:s|d)?\b|\bcounterexample\b|\bfalsif(?:y|ies|ied|ying)\b|反駁|反證|否證|矛盾|相反)/g;
  const mask=(segment)=>segment.replace(support,' <attributed-pos> ').replace(refute,' <attributed-neg> ');
  let out=text.replace(/\b(?:says?|said|reports?|reported|claims?|claimed|states?|stated|argues?|argued|asserts?|asserted|suggests?|suggested|describes?|described)\b[^.!?;；]{0,180}(?=[.!?;；]|$)/g,mask);
  out=out.replace(/\baccording\s+to\b[^.!?;；]{0,180}(?=[.!?;；]|$)/g,mask);
  out=out.replace(/(?:聲稱|宣稱|表示|報告稱|報告指出|主張|認為|指出|寫道|據稱)[^。！？；;]{0,180}(?=[。！？；;]|$)/g,mask);
  out=out.replace(/(?:根據|依據)[^。！？；;]{0,80}[，,][^。！？；;]{0,120}(?=[。！？；;]|$)/g,mask);
  out=out.replace(/(?:根據|依據)[^。！？；;：:]{0,80}[：:][^。！？；;]{0,120}(?=[。！？；;]|$)/g,mask);
  return out;
}
function maskQuotedPolarityPhrases(text){
  const support=/(?:\bsupport(?:s|ed|ing)?\b|\bconfirm(?:s|ed|ing)?\b|\bcorroborat(?:e|es|ed|ing)\b|\bconsistent\s+with\b|支持|佐證|印證|吻合|一致)/g;
  const refute=/(?:\brefut(?:e|es|ed|ing)?\b|\bcontradict(?:s|ed|ing)?\b|\boppose(?:s|d)?\b|\bcounterexample\b|\bfalsif(?:y|ies|ied|ying)\b|反駁|反證|否證|矛盾|相反)/g;
  const mask=(segment)=>segment.replace(support,' <quoted-pos> ').replace(refute,' <quoted-neg> ');
  let out=text
    .replace(/“[^”\n]{0,240}”/g,mask)
    .replace(/‘[^’\n]{0,240}’/g,mask)
    .replace(/「[^」\n]{0,240}」/g,mask)
...
function maskParentheticalPolarityPhrases(text){
  const support=/(?:\bsupport(?:s|ed|ing)?\b|\bconfirm(?:s|ed|ing)?\b|\bcorroborat(?:e|es|ed|ing)\b|\bconsistent\s+with\b|支持|佐證|印證|吻合|一致)/g;
  const refute=/(?:\brefut(?:e|es|ed|ing)?\b|\bcontradict(?:s|ed|ing)?\b|\boppose(?:s|d)?\b|\bcounterexample\b|\bfalsif(?:y|ies|ied|ying)\b|反駁|反證|否證|矛盾|相反)/g;
  const mask=(segment)=>segment.replace(support,' <parenthetical-pos> ').replace(refute,' <parenthetical-neg> ');
  // relationPolarity normalizes NFKC before this function, so Chinese full-width （）
  // arrive here as ASCII parentheses. Support bounded nesting up to depth 3 and 360 chars.
  // If nesting is deeper, unbalanced, multiline or overlong, leave the whole top-level
  // span unmasked so a parser-limit cannot silently create false certainty.
  const ranges=[],stack=[];let topStart=-1,topMaxDepth=0;
  for(let i=0;i<text.length;i++){
    const ch=text[i];
    if(ch==='('){if(stack.length===0){topStart=i;topMaxDepth=1;}stack.push(i);topMaxDepth=Math.max(topMaxDepth,stack.length);continue;}
    if(ch!==')'||!stack.length)continue;
    stack.pop();
    if(stack.length===0&&topStart>=0){const segment=text.slice(topStart,i+1);if(topMaxDepth<=3&&segment.length<=362&&!segment.includes('\n'))ranges.push([topStart,i+1]);topStart=-1;topMaxDepth=0;}
  }
  if(!ranges.length)return text;
  let out='',cursor=0;
  for(const [start,end] of ranges){out+=text.slice(cursor,start);out+=mask(text.slice(start,end));cursor=end;}
  return out+text.slice(cursor);
}
function relationPolarity(text){
  const s=cleanForPolarity(text).toLowerCase();
  const scan=maskParentheticalPolarityPhrases(maskQuotedPolarityPhrases(maskAttributedPolarityPhrases(maskConcessivePolarityPhrases(maskExceptionPolarityPhrases(maskExplicitThenConditionalPolarityPhrases(maskConditionalSentencePolarityPhrases(maskSimpleConditionalPrefixPolarityPhrases(maskConditionalPolarityPhrases(maskHedgedPolarityPhrases(maskNegatedPolarityPhrases(s)))))))))));
...
function normalizeReceipt(r){
  ...
  return {...r,targetRef:clean(r.targetRef),clauseRef:clean(r.clauseRef),lens:clean(r.lens),organ:clean(r.organ||r.sourceOrgan),status:clean(r.status),provenance:clean(r.provenance||r.provenanceFingerprint||r.sourceFingerprint||r.fingerprint),material:clean(r.material||r.summary||r.evidence||r.result),relation:String(r.relation||r.relationToTarget||r.assessment||'').normalize('NFKC').replace(/\r\n?/g,'\n').trim()};
}
...
export function auditReceiptAmbiguity(receipts=[]){
  ...
  return {status,ambiguous,contested,groupCount:groups.size,qualifyingReceiptCount:[...groups.values()].reduce((n,x)=>n+x.length,0),rejectedReceiptCount:rejected.length,rejectedReceipts:rejected,boundary:'Ambiguity/conflict is computed over branch-scoped structurally qualifying executed returns with provenance, material and an explicit relation-to-target field. Bounded English/Chinese negation, insufficiency, modality, conditionality, exception, concessive/contrastive, attribution, paired-quotation and bounded nested-parenthetical masks prevent obvious non-directional, scope-qualified, subordinate, merely reported, merely quoted or parenthetical-only support/refute phrases from creating false unconditional polarity. Generic if/when/as-long-as and 如果/若/倘若/只要 comma-delimited conditional sentences contain both subordinate and same-sentence main-clause directional polarity; explicit if...then and 如果/若/倘若/只要...則/那么/那麼 forms receive the same containment without requiring a comma. English even if is excluded from the generic if matcher: its comma-delimited concessive prefix is contained while a later main-clause or later-sentence directional relation remains visible. Bounded English/Chinese reporting verbs and according-to/根據-style attributions, including 根據/依據 comma or colon delimiters, mask directional vocabulary only within the attributed sentence. Bounded paired quote marks and bounded parenthetical asides up to nesting depth 3 mask directional vocabulary only inside those scopes, while later direct text remains visible. Chinese full-width parentheses are folded by NFKC before the bounded ASCII-parenthesis scan. Newline-bearing, too-deep, overlong or unbalanced parenthetical spans are deliberately left unmasked rather than silently hidden. Explicit opposing polarity across organ labels is preserved as a contested branch. Qualification is necessary but not sufficient for semantic adequacy, quotation/parenthetical attribution, source independence or source quality.'};
}
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {...base,version:`${base.version||'unknown'}+ambiguity-conflict-guard-v0.5.16`,status,unresolved,handoffs:unresolved.map(x=>x.handoff),receiptAmbiguityAudit:audit,handoffResolution:{...(base.handoffResolution||{}),resolved:resolvedBranches.length,open:openBranches.length,contested:contestedBranches.length,ambiguous:audit.ambiguous.length,contestedBranches:contestedBranches.map(x=>x.contest),ambiguousBranches:audit.ambiguous},boundary:`${base.boundary||''} Ambiguity/conflict guard: branch-scoped qualifying executed returns are audited across organ labels. Explicit opposing polarity creates CONTESTED_BY_RECEIPTS and blocks preferred-organ first-receipt closure; an UNSPECIFIED relation creates AMBIGUOUS_BY_RECEIPTS. Obvious negated, explicitly insufficient, explicitly hedged, explicitly conditional, bounded generic conditional-sentence, bounded explicit if-then/若-則 conditional, bounded exception-scoped, bounded concessive/contrastive-prefix, bounded attributed/reported, bounded paired-quotation, or bounded nested-parenthetical support/refute phrases are masked before lexical polarity classification. Generic if/when/as-long-as and 如果/若/倘若/只要 comma-delimited conditional sentences, plus explicit if...then and 如果/若/倘若/只要...則/那么/那麼 forms, mask same-sentence directional polarity through the sentence/semicolon boundary, so condition-scoped main clauses cannot be misread as unconditional closure; later sentences remain visible. English even if is excluded from generic if matching, so its comma-delimited concessive prefix can be masked without hiding an explicit later main-clause or later-sentence direction. Bounded reporting verbs and according-to/根據 attribution scopes, including 根據/依據 comma or colon delimiters, prevent a source's reported support/refute wording from being silently promoted to VAJRA's own unconditional assessment. Bounded paired quote marks and NFKC-normalized parenthetical asides up to depth 3 prevent directional language inside those scopes from being silently promoted while preserving direct text outside them. Newline-bearing, too-deep, overlong or unbalanced parenthetical spans are left unmasked as a conservative parser-limit response, and relation normalization now preserves newline boundaries until parenthetical masking. Distinct organ/provenance labels remain audit metadata rather than proof of independence. This guard is lexical and structural, not general quotation/parenthesis parsing, semantic source attribution or semantic adjudication.`.trim()};
}
