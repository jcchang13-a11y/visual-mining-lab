// VAJRA receipt ambiguity/conflict guard v0.5.9
...
// the same conditional sentence, including the main clause. Bounded explicit if...then and
// 如果/若/倘若/只要...則/那么/那麼 forms are also contained even without a comma.
// Comma-delimited concessive prefixes include even if / 即使 while leaving the later main clause visible.
...
function maskExplicitThenConditionalPolarityPhrases(text){
  ...
  let out=text.replace(/\bif\b[^.!?;；]{0,120}\bthen\b[^.!?;；]{0,120}(?=[.!?;；]|$)/g,mask);
