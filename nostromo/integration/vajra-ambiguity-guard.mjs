// VAJRA receipt ambiguity/conflict guard v0.5.11
...
// concessive/contrastive scope, and attributed/reported polarity. Conditionality/
// exception coverage includes bounded prefix forms without crossing sentence or
// semicolon boundaries. Generic comma-delimited condition prefixes (if/when/as long as
// and 如果/若/倘若/只要) contain directional vocabulary in the same conditional sentence,
// including the main clause. English even if is excluded from the generic if matcher so
// its comma-delimited concessive prefix can be contained without hiding an explicit
// directional main clause. Bounded explicit if...then and 如果/若/倘若/只要...則/那么/那麼
// forms are also contained even without a comma. A later sentence remains visible.
// Attributed source claims are kept auditable but cannot by themselves become VAJRA's
// unconditional branch polarity; a later direct assessment remains visible.
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {...base,version:`${base.version||'unknown'}+ambiguity-conflict-guard-v0.5.11`,status,unresolved,handoffs:unresolved.map(x=>x.handoff),receiptAmbiguityAudit:audit,handoffResolution:{...(base.handoffResolution||{}),resolved:resolvedBranches.length,open:openBranches.length,contested:contestedBranches.length,ambiguous:audit.ambiguous.length,contestedBranches:contestedBranches.map(x=>x.contest),ambiguousBranches:audit.ambiguous},boundary:`${base.boundary||''} Ambiguity/conflict guard: branch-scoped qualifying executed returns are audited across organ labels. Explicit opposing polarity creates CONTESTED_BY_RECEIPTS and blocks preferred-organ first-receipt closure; an UNSPECIFIED relation creates AMBIGUOUS_BY_RECEIPTS. Obvious negated, explicitly insufficient, explicitly hedged, explicitly conditional, bounded generic conditional-sentence, bounded explicit if-then/若-則 conditional, bounded exception-scoped, bounded concessive/contrastive-prefix, or bounded attributed/reported support/refute phrases are masked before lexical polarity classification. Generic if/when/as-long-as and 如果/若/倘若/只要 comma-delimited conditional sentences, plus explicit if...then and 如果/若/倘若/只要...則/那么/那麼 forms, mask same-sentence directional polarity through the sentence/semicolon boundary, so condition-scoped main clauses cannot be misread as unconditional closure; later sentences remain visible. English even if is excluded from generic if matching, so its comma-delimited concessive prefix can be masked without hiding an explicit later main-clause or later-sentence direction. Bounded reporting verbs and according-to/根據 attribution scopes prevent a source's reported support/refute wording from being silently promoted to VAJRA's own unconditional assessment; a later direct assessment remains visible. Distinct organ/provenance labels remain audit metadata rather than proof of independence. This guard is lexical and structural, not semantic adjudication.`.trim()};
}
