// VAJRA receipt ambiguity/conflict guard v0.5.3
...
// phrases, including explicit insufficiency, modality, conditionality and exception
// scope phrases. Conditionality/exception coverage includes bounded prefix forms
// without crossing sentence or semicolon boundaries. This is deterministic audit
// logic, not semantic adjudication, source-independence proof or source-quality scoring.
...
function relationPolarity(text){
  ...
  const scan=maskExceptionPolarityPhrases(maskConditionalPolarityPhrases(maskHedgedPolarityPhrases(maskNegatedPolarityPhrases(s))));
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {...base,version:`${base.version||'unknown'}+ambiguity-conflict-guard-v0.5.3`,status,unresolved,handoffs:unresolved.map(x=>x.handoff),receiptAmbiguityAudit:audit,handoffResolution:{...(base.handoffResolution||{}),resolved:resolvedBranches.length,open:openBranches.length,contested:contestedBranches.length,ambiguous:audit.ambiguous.length,contestedBranches:contestedBranches.map(x=>x.contest),ambiguousBranches:audit.ambiguous},boundary:`${base.boundary||''} Ambiguity/conflict guard: branch-scoped qualifying executed returns are audited across organ labels. Explicit opposing polarity creates CONTESTED_BY_RECEIPTS and blocks preferred-organ first-receipt closure; an UNSPECIFIED relation creates AMBIGUOUS_BY_RECEIPTS. Obvious negated, explicitly insufficient, explicitly hedged, explicitly conditional, or bounded exception-scoped support/refute phrases are masked before lexical polarity classification. Prefix exception/condition scans do not cross sentence or semicolon boundaries. Distinct organ/provenance labels remain audit metadata rather than proof of independence. This guard is lexical and structural, not semantic adjudication.`.trim()};
}
