// VAJRA receipt ambiguity/conflict guard v0.5.6
...
// prefixes (if/when/as long as and 如果/若/倘若/只要) mask directional vocabulary only inside the
// subordinate prefix, so a later main-clause directional statement remains visible.
// Concessive/contrastive coverage follows the same bounded prefix principle. This is deterministic
...
function relationPolarity(text){
  ...
  const scan=maskConcessivePolarityPhrases(maskExceptionPolarityPhrases(maskSimpleConditionalPrefixPolarityPhrases(maskConditionalPolarityPhrases(maskHedgedPolarityPhrases(maskNegatedPolarityPhrases(s))))));
...
export function applyGuardedHandoffResults(vajraResult,receipts=[],engine=globalThis.VajraEngine){
  ...
  return {...base,version:`${base.version||'unknown'}+ambiguity-conflict-guard-v0.5.6`,status,unresolved,handoffs:unresolved.map(x=>x.handoff),receiptAmbiguityAudit:audit,handoffResolution:{...(base.handoffResolution||{}),resolved:resolvedBranches.length,open:openBranches.length,contested:contestedBranches.length,ambiguous:audit.ambiguous.length,contestedBranches:contestedBranches.map(x=>x.contest),ambiguousBranches:audit.ambiguous},boundary:`${base.boundary||''} Ambiguity/conflict guard: branch-scoped qualifying executed returns are audited across organ labels. Explicit opposing polarity creates CONTESTED_BY_RECEIPTS and blocks preferred-organ first-receipt closure; an UNSPECIFIED relation creates AMBIGUOUS_BY_RECEIPTS. Obvious negated, explicitly insufficient, explicitly hedged, explicitly conditional, bounded generic conditional-prefix, bounded exception-scoped, or bounded concessive/contrastive-prefix support/refute phrases are masked before lexical polarity classification. Generic if/when/as-long-as and 如果/若/倘若/只要 prefix scans stop at the first comma, so later main-clause polarity remains visible. Condition/exception prefix scans do not cross sentence or semicolon boundaries; concessive/contrastive-prefix scans stop at the first comma so later main-clause polarity remains visible. Distinct organ/provenance labels remain audit metadata rather than proof of independence. This guard is lexical and structural, not semantic adjudication.`.trim()};
}
