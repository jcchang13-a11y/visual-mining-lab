...
if(intake.acceptedPacketIds?.length!==3||intake.returnLedger?.newlyAcceptedCount!==3)failures.push({type:'RETURN_LEDGER_NOT_EXPOSED',intake});
...
// Same-intake mutated replay adversary: a second content/provenance variant for the same issued packet must not amplify one organ's vote.
const mutatedReplay=[
  goodReturns[0],
...
// Cross-intake replay adversary: after an earlier intake accepts a packet, a delayed/mutated later return must not be accepted again when the ledger is carried forward.
...
const secondIntake=ingestConflictEscalationReturns(out,[delayedMutatedReturn],{acceptedPacketIds:firstIntake.acceptedPacketIds});
if(secondIntake.acceptedReturns!==0||secondIntake.quarantinedReturns!==1||!secondIntake.quarantine[0]?.reasons?.includes('PACKET_PREVIOUSLY_RETURNED'))failures.push({type:'CROSS_INTAKE_PACKET_REPLAY_NOT_CONTAINED',firstIntake,secondIntake});
if(secondIntake.returnLedger?.previouslyAcceptedCount!==1||secondIntake.acceptedPacketIds?.length!==1)failures.push({type:'CROSS_INTAKE_RETURN_LEDGER_NOT_PRESERVED',secondIntake});
if(secondIntake.missingPacketIds?.includes(goodReturns[0].packetId))failures.push({type:'PREVIOUSLY_ACCEPTED_PACKET_FALSELY_MARKED_MISSING',secondIntake});
