...
check(globalThis.VajraEngine.receiptUncertaintyGuardVersion==='0.3.3','GUARD_VERSION_MISSING',globalThis.VajraEngine.receiptUncertaintyGuardVersion);
...
for(const [label,receipt,reason,counter] of [
  ...
  check(applied.version==='0.3.3',`${label}_VERSION`,applied.version);
...
const gut=globalThis.GutEngine.digest({vajra:globalThis.VajraEngine.applyHandoffResults(base,[longNoncommittal])},{source:'VAJRA_UNCERTAINTY_GUT_REGRESSION',inheritedSubstrates:[target]});
check(!gut.summary.includes(target),'GUT_TARGET_ECHO_AFTER_RELATION_GUARD',gut.summary);
check(gut.ingested>0&&gut.absorbed>=0,'GUT_REGRESSION_FAILED',{ingested:gut.ingested,absorbed:gut.absorbed});
const result={schema:'nostromo-vajra-uncertainty-test/v0.3.3',completedAt:new Date().toISOString(),status:failures.length?'FAIL':'PASS',guards:{explicitUncertaintyCannotClose:true,negatedSupportCannotClose:true,negatedRefuteCannotClose:true,chineseUncertaintyCannotClose:true,longUnspecifiedRelationCannotClose:true,positiveSupportStillCloses:true,explicitRefuteStillCloses:true,gutRegression:true},finding:{classification:'RELATION_SPECIFICITY_GUARD_ADDED',detail:'A structurally complete and sufficiently long handoff receipt could previously close a VAJRA branch even when its relation merely discussed the target without saying whether the material supports or refutes it. v0.3.3 blocks both explicit uncertainty and long relation-unspecified returns, preserving them in the audit log while leaving the branch open.'},crossOrgan:{gut:{ingested:gut.ingested,absorbed:gut.absorbed,quarantined:gut.quarantined,summarySample:gut.summary.slice(0,360)}},failures,boundary:'PASS certifies only a bounded lexical anti-false-certainty guard over handoff receipt relations plus a GUT regression. Closure now requires an explicit SUPPORTS, REFUTES, or MIXED polarity signal after uncertainty screening. It does not semantically judge relevance, evidence quality, truth, or whether one source should outweigh another.'};
