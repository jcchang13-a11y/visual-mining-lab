// NOSTROMO active executor loop v0.6
...
export async function runActiveExecutorLoop({rounds=10,seed='NOSTROMO active integration',mineQuery='NOSTROMO',verifyUrl='https://github.com/jcchang13-a11y/visual-mining-lab'}={}){
  ...
  for(let round=1;round<=total;round++){
    ...
    const externalConnector=round===1?connectorEvidence:{
      schema:'nostromo-connector-evidence-carry/v0.6',
      status:'CARRIED',
    ...
    const executorPayload={round,shrooming,muther,droplet,externalConnector,vajra};
  ...
  return {
    schema:'nostromo-active-executor-loop/v0.6',
    ...
    boundary:'Certifies closed-loop routing for SHROOMING local deterministic sandbox, MU/TH/UR repository mining, DROPLET explicit-URL verification, VAJRA and GUT, plus acceptance and digestion of evidence produced by externally executed Google Drive query mining and public web search. It does not certify live SHROOMING, exhaustive whole-Drive mining, or that GitHub Actions itself can call those connectors.'
  };
}
