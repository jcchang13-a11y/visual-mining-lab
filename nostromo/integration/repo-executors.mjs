...
export function mutherLexicalVeins(candidates,{query='',threshold=0.18,maxAuditEdges=48}={}){
  ...
  for(let i=0;i<items.length;i++)for(let j=i+1;j<items.length;j++){
    ...
    const edge={aFingerprint:items[i].contentFingerprint,bFingerprint:items[j].contentFingerprint,aPath:items[i].path,bPath:items[j].path,aSourceFamily:items[i].sourceFamily,bSourceFamily:items[j].sourceFamily,crossFamily:items[i].sourceFamily!==items[j].sourceFamily,similarity:Number(similarity.toFixed(4))};
    allEdges.push(edge);union(i,j);
  }
  const groups=new Map();for(let i=0;i<items.length;i++){const r=find(i);if(!groups.has(r))groups.set(r,[]);groups.get(r).push(items[i]);}
  const veins=[...groups.values()].filter(g=>g.length>1).map((members,index)=>{
    const fps=new Set(members.map(x=>x.contentFingerprint));
    const edges=allEdges.filter(e=>fps.has(e.aFingerprint)&&fps.has(e.bFingerprint));
    const sourceFamilies=[...new Set(members.map(x=>x.sourceFamily))].sort();
    return {id:`lexical-vein-${index+1}`,memberCount:members.length,sourceFamilyCount:sourceFamilies.length,crossFamily:sourceFamilies.length>1,sourceFamilies,paths:[...new Set(members.map(x=>x.path))].sort(),fingerprints:members.map(x=>x.contentFingerprint),edgeCount:edges.length,maxAffinity:edges.length?Math.max(...edges.map(x=>x.similarity)):null,minAffinity:edges.length?Math.min(...edges.map(x=>x.similarity)):null};
  }).sort((a,b)=>b.memberCount-a.memberCount||b.sourceFamilyCount-a.sourceFamilyCount||a.id.localeCompare(b.id));
  return {threshold:t,edgeCount:allEdges.length,veinCount:veins.length,crossFamilyVeinCount:veins.filter(x=>x.crossFamily).length,unclusteredCount:items.length-veins.reduce((n,v)=>n+v.memberCount,0),veins,edges:allEdges.sort((a,b)=>b.similarity-a.similarity).slice(0,Math.max(1,Math.min(100,Number(maxAuditEdges)||48)))};
}
