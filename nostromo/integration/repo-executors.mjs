// NOSTROMO repository-native executors v1.5
...
function stripQueryForVein(text,query){const normalized=normalizeCandidateText(text);const q=normalizeCandidateText(query);if(!q)return normalized;return normalized.split(q).join(' ').replace(/\s+/g,' ').trim();}
export function mutherVeinAffinity(a,b,query=''){return mutherCandidateSimilarity(stripQueryForVein(a,query),stripQueryForVein(b,query));}
export function mutherLexicalVeins(candidates,{query='',threshold=0.18,maxAuditEdges=48}={}){
  const items=Array.isArray(candidates)?candidates:[];
  const t=Math.max(0.08,Math.min(0.7,Number(threshold)||0.18));
  const parents=items.map((_,i)=>i);
  const find=i=>{while(parents[i]!==i){parents[i]=parents[parents[i]];i=parents[i];}return i;};
  const union=(a,b)=>{const ra=find(a),rb=find(b);if(ra!==rb)parents[rb]=ra;};
  const allPairs=[];
  const allEdges=[];
  for(let i=0;i<items.length;i++)for(let j=i+1;j<items.length;j++){
    const similarity=mutherVeinAffinity(items[i].snippet,items[j].snippet,query);
    const pair={aIndex:i,bIndex:j,aFingerprint:items[i].contentFingerprint,bFingerprint:items[j].contentFingerprint,aPath:items[i].path,bPath:items[j].path,aSourceFamily:items[i].sourceFamily,bSourceFamily:items[j].sourceFamily,crossFamily:items[i].sourceFamily!==items[j].sourceFamily,similarity:Number(similarity.toFixed(4))};
    allPairs.push(pair);
    if(similarity<t)continue;
    allEdges.push(pair);union(i,j);
  }
  const groups=new Map();for(let i=0;i<items.length;i++){const r=find(i);if(!groups.has(r))groups.set(r,[]);groups.get(r).push(i);}
  const connected=[...groups.values()].filter(g=>g.length>1).map((memberIndexes,index)=>{
    const memberSet=new Set(memberIndexes);
    const members=memberIndexes.map(i=>items[i]);
    const edges=allEdges.filter(e=>memberSet.has(e.aIndex)&&memberSet.has(e.bIndex));
    const pairAffinities=allPairs.filter(e=>memberSet.has(e.aIndex)&&memberSet.has(e.bIndex));
    const sourceFamilies=[...new Set(members.map(x=>x.sourceFamily))].sort();
    const minPairAffinity=pairAffinities.length?Math.min(...pairAffinities.map(x=>x.similarity)):null;
    const maxPairAffinity=pairAffinities.length?Math.max(...pairAffinities.map(x=>x.similarity)):null;
    const cohesive=pairAffinities.length===memberIndexes.length*(memberIndexes.length-1)/2&&pairAffinities.every(x=>x.similarity>=t);
    return {id:`lexical-component-${index+1}`,memberCount:members.length,sourceFamilyCount:sourceFamilies.length,crossFamily:sourceFamilies.length>1,sourceFamilies,paths:[...new Set(members.map(x=>x.path))].sort(),fingerprints:members.map(x=>x.contentFingerprint),edgeCount:edges.length,maxAffinity:edges.length?Math.max(...edges.map(x=>x.similarity)):null,minAffinity:edges.length?Math.min(...edges.map(x=>x.similarity)):null,minPairAffinity,maxPairAffinity,cohesive,bridgeRisk:!cohesive};
  }).sort((a,b)=>b.memberCount-a.memberCount||b.sourceFamilyCount-a.sourceFamilyCount||a.id.localeCompare(b.id));
  const veins=connected.filter(x=>x.cohesive).map((x,index)=>({...x,id:`lexical-vein-${index+1}`}));
  const bridgeRisks=connected.filter(x=>x.bridgeRisk).map((x,index)=>({...x,id:`lexical-bridge-risk-${index+1}`}));
  const clusteredFingerprints=new Set(veins.flatMap(v=>v.fingerprints));
  return {threshold:t,edgeCount:allEdges.length,veinCount:veins.length,crossFamilyVeinCount:veins.filter(x=>x.crossFamily).length,bridgeRiskCount:bridgeRisks.length,crossFamilyBridgeRiskCount:bridgeRisks.filter(x=>x.crossFamily).length,unclusteredCount:items.filter(x=>!clusteredFingerprints.has(x.contentFingerprint)).length,veins,bridgeRisks,edges:allEdges.map(({aIndex,bIndex,...edge})=>edge).sort((a,b)=>b.similarity-a.similarity).slice(0,Math.max(1,Math.min(100,Number(maxAuditEdges)||48)))};
}
...
export async function mutherMineRepo({query='',limit=12,nearDuplicateThreshold=0.88,maxWindowsPerFile=4,minWindowGap=320,veinThreshold=0.18}={}){
