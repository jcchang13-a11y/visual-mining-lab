// ZENOMORPH big-meal runner v0.2.1
...
function neutralChunks(text,{target=3200,min=900}={}){
  ...
  for(const p of paras){ if(size+p.length>target&&size>=min){chunks.push(buf.join('\n\n'));buf=[];size=0;} buf.push(p);size+=p.length+2; }
...
function mutherWholeText(chunks){
  ...
  const affinities=[];
  for(let i=0;i<chunks.length;i++)for(let j=i+2;j<chunks.length;j++){ const similarity=mutherCandidateSimilarity(chunks[i],chunks[j]); if(similarity>=0.08)affinities.push({a:i,b:j,similarity:Number(similarity.toFixed(4))}); }
  affinities.sort((a,b)=>b.similarity-a.similarity);
  const recombinations=affinities.slice(0,12).map((edge,k)=>({id:`meal-recombination-${k+1}`,parents:[edge.a,edge.b],affinity:edge.similarity,mutation:`[A${edge.a}] ${compact(chunks[edge.a],700)}\n[B${edge.b}] ${compact(chunks[edge.b],700)}`,boundary:'STRUCTURAL RECOMBINATION CANDIDATE ONLY; NOT A CLAIM, SUMMARY, OR INCORPORATED CAPABILITY'}));
...
async function tryAcquire(url,manifest){
  const response=await fetch(url,{redirect:'follow',headers:{'user-agent':'Mozilla/5.0 ZENOMORPH-DROPLET/1.2','accept':'text/html,application/xhtml+xml,application/xml,text/xml,application/pdf;q=0.9,*/*;q=0.5'}});
  const contentType=String(response.headers.get('content-type')||'').toLowerCase();
  if(!response.ok)return {ok:false,url,responseStatus:response.status,contentType,reason:'HTTP'};
  const bytes=Buffer.from(await response.arrayBuffer()); const isPdf=contentType.includes('pdf')||bytes.subarray(0,5).toString()==='%PDF-'; let text;
  try{text=isPdf?await pdfToText(bytes):htmlToText(bytes.toString('utf8'));}catch(error){return {ok:false,url,responseStatus:response.status,contentType,bytes:bytes.length,reason:`EXTRACT:${String(error?.message||error).slice(0,240)}`};}
  const textIdentity=normalizedIdentity(text); const titleVerified=textIdentity.includes(normalizedIdentity(manifest.title)); const authorVerified=textIdentity.includes(normalizedIdentity(manifest.author)); const doiVerified=text.toLowerCase().replace(/\s+/g,'').includes(String(manifest.doi||'').toLowerCase().replace(/\s+/g,'')); const bodyLargeEnough=text.length>12000;
  return {ok:titleVerified&&authorVerified&&doiVerified&&bodyLargeEnough,url,responseStatus:response.status,contentType,format:isPdf?'pdf':contentType.includes('xml')?'xml':'html',bytes:bytes.length,sourceSha256:sha256(bytes),text,textCharCount:text.length,titleVerified,authorVerified,doiVerified,bodyLargeEnough,reason:(titleVerified&&authorVerified&&doiVerified&&bodyLargeEnough)?null:'IDENTITY_OR_BODY'};
}

async function dropletAcquire(manifest){
  const candidates=[manifest.source?.primaryHtml,manifest.source?.primary,manifest.source?.repositoryPdf,manifest.source?.publisherPdf,manifest.source?.publicArchiveXml].filter(Boolean);
  if(!candidates.length)throw new Error('MEAL_SOURCE_REQUIRED'); const attempts=[];
  for(const url of [...new Set(candidates)]){ try{const result=await tryAcquire(url,manifest);const {text,...audit}=result;attempts.push(audit);if(result.ok)return {...result,attempts};}catch(error){attempts.push({url,ok:false,reason:`FETCH:${String(error?.message||error).slice(0,240)}`});} }
  throw new Error(`DROPLET_ALL_SOURCES_REJECTED:${JSON.stringify(attempts)}`);
}

async function loadEngines(){ await import('../gut/gut-engine.js'); await import('../vajra/vajra-engine.js'); if(!globalThis.GutEngine) throw new Error('GUT_UNAVAILABLE'); if(!globalThis.VajraEngine) throw new Error('VAJRA_UNAVAILABLE'); }
...
export async function runBigMeal({manifestPath='nostromo/research/big-meals/pasquinelli-2026.json',outputPath=null}={}){
  ...
  const acquired=await dropletAcquire(manifest); const text=acquired.text;
  ...
  const chunks=neutralChunks(text); const muther=mutherWholeText(chunks); await loadEngines();
