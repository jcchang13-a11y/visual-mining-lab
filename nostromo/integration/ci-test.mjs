// NOSTROMO repository-native integration CI v1.2
...
try{
  ...
  if(active.connectorHandoff?.status!=='ACCEPTED'||active.connectorHandoff?.actionsAccepted!==3)failures.push({type:'CONNECTOR_HANDOFF_FAIL',detail:active.connectorHandoff});
  ...
  const result={
    schema:'nostromo-integration-ci/v1.2',completedAt:new Date().toISOString(),status:out.status==='PASS'&&active.status==='PASS'&&connector.status==='ACCEPTED'&&formal.status==='PASS'&&failures.length===0?'PASS':'FAIL',rounds:out.rounds,
