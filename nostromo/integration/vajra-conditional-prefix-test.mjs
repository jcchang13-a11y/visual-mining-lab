...
for(const [name,relation,expected] of cases){
  ...
  if(expected==='UNSPECIFIED'&&audit.status!=='AMBIGUITY_FOUND')failures.push({type:'CONDITIONAL_PREFIX_DID_NOT_BLOCK_CLOSURE',name,auditStatus:audit.status});
}

const result={
  schema:'nostromo-vajra-conditional-prefix-test/v0.1',
  status:failures.length?'FAIL':'PASS',
  guard:'VAJRA ambiguity/conflict guard v0.5.2',
  capability:'BOUNDED_ENGLISH_CONDITION_PREFIX_SCOPE_CONTAINMENT',
  observations,
  failures,
  boundary:'This adversarial test covers bounded English condition-prefix forms before support/refute vocabulary and verifies that masking does not cross sentence or semicolon boundaries. It is lexical/structural scope containment, not general logical parsing or semantic condition adjudication.'
};
