/* ZENOMORPH / NOSTROMO GUT capability evidence role-separation gate v0.1
 * Prevents semantic evidence laundering by reusing the same substantive token
 * as provenance, permission, and interface/contract evidence.
...
export function assessCapabilityEvidenceRoleSeparation(candidate){
  const base={
    schema:'zenomorph-gut-capability-evidence-role-separation/v0.1',
  ...
  for(let i=0;i<active.length;i++){
    for(let j=i+1;j<active.length;j++){
      ...
      for(const atom of atomsA){
        if(atomsB.has(atom)){
          return {...base,status:'HOLD',classification:'EVIDENCE_ROLE_COLLISION',reason:'cross-role-evidence-reuse',roles:[roleA,roleB],reusedAtom:atom.slice(0,160)};
        }
      }
    }
  }
  return {...base,status:'PASS',classification:'EVIDENCE_ROLES_SEPARATED',reason:'no-substantive-evidence-token-is-reused-across-source-permission-interface-roles',roleAtomCounts:Object.fromEntries(Object.entries(roles).map(([k,v])=>[k,v.size])),nextStage:'capability admission / sandbox gates remain authoritative'};
}

export const capabilityEvidenceRoleSeparationBoundary=Object.freeze({
  version:'0.1',normalization:'Unicode NFKC + whitespace collapse + lowercase',rejectsCrossRoleTokenReuse:true,authorizationGranted:false,executionGranted:false,installationGranted:false,bodyAdmissionGranted:false
});
