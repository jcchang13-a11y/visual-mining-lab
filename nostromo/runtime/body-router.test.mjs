...
assert.equal(state.stable.capabilities.length,1);
assert.equal(state.growing.candidates.length,0);
...
assert.notEqual(envelopeOnly.comparison.stableEnvelopeFingerprint,envelopeOnly.comparison.shadowEnvelopeFingerprint);
assert.equal(envelopeOnly.comparison.stableBehaviorFingerprint,envelopeOnly.comparison.shadowBehaviorFingerprint);
assert.equal(envelopeOnly.comparison.differs,false);
assert.deepEqual(behavioralProjection(envelopeOnly.official),{answer:'same',route:['a','b']});

// Semantically equal behavioral objects with different insertion order must not manufacture evidence.
const keyOrderState=createRuntimeState({stableCapabilities:[{id:'baseline'}]});
const keyOrderOnly=await runTask({
  task:{kind:'probe',payload:'key-order'},
  state:keyOrderState,
  stableExecutor:async()=>({result:{answer:'same',metrics:{coverage:4,similarity:0.91}}}),
  growingExecutor:async()=>({result:{metrics:{similarity:0.91,coverage:4},answer:'same'}})
});
...
registerCandidate(state,{id:'candidate-1',kind:'route',payload:{routeBias:'retain-provenance'},evidence:{
  isolated_generation:true,stress:true,provenance:true,cross_organ:true,regression:true,held_out:true,cross_food_transfer:false,delayed_retest:true
}});
...
const ablation=await runAblationPair({
  task:{kind:'held-out',payload:'orthogonal-food'},state,candidateId:'candidate-1',
  executor:async({body,candidate})=>({result:{route:body.candidates.some(x=>x.id==='candidate-1')?'candidate-route':'control-route',candidateSeen:Boolean(candidate)}})
});
assert.equal(ablation.control.result.route,'control-route');
assert.equal(ablation.exposed.result.route,'candidate-route');
...
// Ablation envelopes and key order may differ while behavior is identical; that is not a causal effect.
const ablationEnvelopeState=createRuntimeState({stableCapabilities:[{id:'baseline'}],candidates:[{id:'candidate-envelope',evidence:{}}]});
const ablationEnvelopeOnly=await runAblationPair({
  task:{kind:'held-out',payload:'envelope-only'},state:ablationEnvelopeState,candidateId:'candidate-envelope',
  executor:async({mode})=>mode==='ablation-control'
    ? {mode,authority:'CONTROL',result:{answer:'same',metrics:{a:1,b:2}}}
    : {authority:'EXPOSED',mode,result:{metrics:{b:2,a:1},answer:'same'}}
});
assert.notEqual(ablationEnvelopeOnly.receipt.controlEnvelopeFingerprint,ablationEnvelopeOnly.receipt.exposedEnvelopeFingerprint);
assert.equal(ablationEnvelopeOnly.receipt.controlFingerprint,ablationEnvelopeOnly.receipt.exposedFingerprint);
