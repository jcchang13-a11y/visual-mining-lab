/* VAJRA ENGINE v0.2.0 — deterministic proposition-aware dismantling */
...
function nextQuestion(lens,position){const target=clean(position).slice(0,180);return `${instructionFor(lens)}：若對「${target}」做這個檢查，哪個前提最可能先失效？`;}
function runVajra(text,maxRounds=6){
  const base=detectPosition(text),type=classifyPosition(base.position),trace=[],seen=new Set(),unresolved=[];
  ...
  for(let i=0;i<cap;i++){
    ...
    if(seen.has(signature))return {status:'INFORMATION_SATURATION',version:'0.2.0',positionType:type,trace,unresolved,lensesUsed:[...new Set(trace.map(x=>x.lens))],boundary:'Deterministic heuristic dismantling. Saturation means the current rule set has stopped producing a new inspection path; it is not proof that the claim is settled.'};
    seen.add(signature);
    const question=nextQuestion(lens,base.position);const branch={lens,question,status:'UNRESOLVED'};unresolved.push(branch);
    trace.push({round:i+1,position:current,positionType:type,assumptions,lens,action:instructionFor(lens),question,next:`暫不定案；保留 ${lens} 分支待證據或反例回填。`});
    current=question;
  }
  return {status:'DEPTH_BOUND',version:'0.2.0',positionType:type,trace,unresolved,lensesUsed:[...new Set(trace.map(x=>x.lens))],boundary:'Deterministic proposition-aware dismantling only. Classification and lens choice are auditable heuristics, not semantic truth; unresolved branches must be supplied with evidence or counterexamples by other organs.'};
}
