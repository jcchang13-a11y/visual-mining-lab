/* VAJRA ENGINE v0.2.2 — deterministic proposition-aware dismantling */
...
function resultEnvelope(status,type,targetRef,trace,unresolved){return {status,version:'0.2.2',positionType:type,targetRef,trace,unresolved,lensesUsed:[...new Set(trace.map(x=>x.lens))],boundary:status==='INFORMATION_SATURATION'?'Deterministic heuristic dismantling. Saturation means the current rule set has stopped producing a new inspection path; it is not proof that the claim is settled.':'Deterministic proposition-aware dismantling only. Classification and lens choice are auditable heuristics, not semantic truth; unresolved branches must be supplied with evidence or counterexamples by other organs. The inherited proposition is intentionally not copied into VAJRA output; targetRef preserves referential continuity while the caller retains the source substrate and provenance.'};}
function runVajra(text,maxRounds=6){
  ...
  for(let i=0;i<cap;i++){
    ...
    trace.push({round:i+1,position:`ref:${targetRef}`,positionType:type,targetRef,assumptions,lens,action:instructionFor(lens),question,next:`暫不定案；保留 ${lens} 分支待證據或反例回填。`});
    current=question;
  }
