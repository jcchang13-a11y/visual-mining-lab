// VAJRA mutation review receipt issuer v0.1
// Produces a module-bound contradiction/counterexample review receipt for MUTHER mutation candidates.
...
const MODULE_ID = 'nostromo/vajra/mutation-review-receipt@0.1';
const SCHEMA = 'zenomorph-vajra-mutation-review-receipt/v0.1';
...
function receiptPayload(receipt) {
  ...
  return {
    ...
    reviewWitness: {
      ...
      evidenceRefs,
      checks: {
        contradictionChecked: receipt?.reviewWitness?.checks?.contradictionChecked === true,
        counterexampleChecked: receipt?.reviewWitness?.checks?.counterexampleChecked === true,
...
export function issueVajraMutationReviewReceipt({ candidateId, lineageFingerprint, derivationFingerprint, provenance, reviewRunId, evidenceRefs, reviewWitnessFingerprint } = {}) {
  ...
  const base = {
    ...
    reviewWitness: {
      organ: 'VAJRA',
      kind: 'CONTRADICTION_COUNTEREXAMPLE_REVIEW',
      candidateId: canonical(candidateId),
      lineageFingerprint: canonical(lineageFingerprint),
      derivationFingerprint: canonical(derivationFingerprint),
      reviewRunId: canonical(reviewRunId),
      evidenceRefs: refs || [],
      checks: { contradictionChecked:true, counterexampleChecked:true, provenanceChecked:true },
  ...
  if (!payload || !payload.candidateId || !payload.lineageFingerprint || !payload.derivationFingerprint || !payload.provenance || !payload.reviewRunId || !payload.reviewWitness.reviewWitnessFingerprint) {
    return { schema:SCHEMA, issuerModule:MODULE_ID, organ:'VAJRA', status:'HOLD', reason:'VAJRA_REVIEW_RECEIPT_INPUT_INCOMPLETE', incorporationAuthorized:false };
  }
  return { ...base, issuerFingerprint:fingerprint('vajra-mutation-review-v1', payload) };
}
...
export function verifyVajraMutationReviewReceipt(receipt) {
  ...
  if (!payload) return { ok:false, reason:'VAJRA_ISSUER_EVIDENCE_REQUIRED' };
  const derived = fingerprint('vajra-mutation-review-v1', payload);
