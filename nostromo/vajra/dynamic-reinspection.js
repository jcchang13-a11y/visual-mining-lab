...
(function(root){
  ...
  const canonicalIdentity=v=>validIdentity(v)?v.trim().normalize('NFKC'):null;
  ...
  function quarantineNovelty(branch){
    ...
    const baselineKeys=new Set(baselineTupleAligned?baselineKeyList.map(canonicalIdentity):[]);
    const baselineFingerprints=new Set(baselineTupleAligned?baselineFingerprintList.map(canonicalIdentity):[]);
    const baselineProvenances=new Set(baselineTupleAligned?baselineProvenanceList.map(canonicalIdentity):[]);
    const novelKeys=currentTupleAligned?currentKeyList.filter(v=>!baselineKeys.has(canonicalIdentity(v))):[];
    const novelFingerprints=currentTupleAligned?currentFingerprintList.filter(v=>!baselineFingerprints.has(canonicalIdentity(v))):[];
    const novelProvenances=currentTupleAligned?currentProvenanceList.filter(v=>!baselineProvenances.has(canonicalIdentity(v))):[];
    const novelTuples=[];
    if(tuplePairingValid){
      for(let i=0;i<currentKeyList.length;i++){
        ...
        if(!baselineKeys.has(canonicalIdentity(key))&&!baselineFingerprints.has(canonicalIdentity(fingerprint))&&!baselineProvenances.has(canonicalIdentity(provenance))) novelTuples.push({key,fingerprint,provenance,index:i});
      }
    }
  ...
  function reactivatedInspection(contested,novelty){
    return {trigger:'NOVEL_POST_QUARANTINE_EVIDENCE',targetRef:contested.targetRef,clauseRef:contested.clauseRef,priorLens:contested.lens,lens:'source_quality',preferredOrgan:'DROPLET',status:'OPEN',reason:'A previously quarantined metabolic contest now contains at least one originally co-indexed evidence item whose identity, content fingerprint, and provenance identity are all valid and novel relative to the explicit quarantine baseline after bounded presentation-alias normalization. VAJRA reopens only clause-scoped source-quality inspection so the new material can be checked without treating novelty as truth.',provenancePolicy:'Preserve the quarantine baseline, every prior contest/triage record, original tuple positions, and the raw newly observed evidence key/fingerprint/provenance tuples. Bounded trim + Unicode NFKC canonicalization is used only for duplicate/novelty comparison and does not rewrite stored provenance. Reactivation records provenance-bound information gain but does not claim source independence, correctness, semantic novelty, or resolution.',novelty:{novelEvidenceKeys:novelty.novelKeys,novelEvidenceFingerprints:novelty.novelFingerprints,novelEvidenceProvenances:novelty.novelProvenances,novelEvidencePairs:novelty.novelPairs,novelEvidenceTuples:novelty.novelTuples},boundary:'Reactivation requires one fully valid tuple at one preserved original array index with a canonically new evidence key, content fingerprint, and provenance identity relative to a fully valid aligned baseline. Identity members must contain visible non-control content. Novelty comparison trims outer whitespace and Unicode NFKC-normalizes compatibility forms only; it does not lowercase or perform semantic aliasing. Invalid members are never filtered or compacted into synthetic tuples. Split novelty, borrowed baseline provenance, presentation-only aliases, missing provenance, malformed members, tuple misalignment, replay, duplicate aliases, or missing baseline stays quarantined. Reopening does not erase the prior HOLD and does not authorize capability incorporation.'};
  }
  ...
  function wrapped(vajraResult,receipts=[]){
    ...
    return {...out,nextInspection,dynamicReinspection:{version:'0.9',triggered:['CONTESTED_RETURN','REPEATED_SOURCE_CONTEST','REPEATED_METABOLIC_CONTEST','NOVEL_POST_QUARANTINE_EVIDENCE'].includes(nextInspection?.trigger),quarantineDeferred,policy:'RUNNABLE_CONFLICT_FIRST_WITH_POSITION_PRESERVING_PROVENANCE_TUPLE_GATED_QUARANTINE_REACTIVATION',boundary:'A qualifying inter-organ receipt conflict changes VAJRA next-step behavior. Repeated metabolic_contamination conflict is locally quarantined and preserved. Quarantine can become runnable again only when explicit aligned baseline comparison proves at least one originally co-indexed, fully valid evidence item whose key, content fingerprint, and provenance identity are all previously unseen after bounded trim + Unicode NFKC presentation-alias normalization. Raw evidence and provenance values remain preserved; canonicalization is comparison-only and does not lowercase or infer semantic identity. Identity members must carry visible non-control content. Invalid array members are never independently filtered into synthetic tuple alignment. Split novelty, borrowed baseline provenance, presentation-only aliases, malformed or missing members, tuple misalignment, replay, duplicate pollution, or missing baseline cannot reopen it. Reactivation returns only to clause-scoped DROPLET source-quality inspection and does not treat novelty as truth. Terminal quarantine cannot starve unrelated contested branches. Missing, malformed, replayed, or otherwise rejected receipts cannot trigger escalation.'}};
  }
